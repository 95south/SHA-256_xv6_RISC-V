...
int run_single_test(const char* text, int text_len) {
    char final_hash_hex[SHA256_HEX_SIZE];
    ...
    while (offset < text_len) {
        int chunk_len = text_len - offset < CHUNK_SIZE ? text_len - offset : CHUNK_SIZE;
        sha256_append(&sha, &text[offset], chunk_len);
        offset += chunk_len;
    }
...
int main(int argc, char *argv[]) {
    ...
    while ((bytes_read = read(fd, buffer, CHUNK_SIZE)) > 0) {
        printf("%ld\n", bytes_read);
        sha256_append(&sha, (unsigned char *)buffer, bytes_read);
    }
    ...
    // Finalize hash and print as hexadecimal
    char final_hash_hex[SHA256_HEX_SIZE];
    sha256_finalize_hex(&sha, final_hash_hex);
